...
def show_range_image(frame, lidar_name):
    ...
    # From the documentation, we know that the inner dimensions of a range image cell are [range, intensity, elongation, is_in_no_label_zone].
    range_ch     = ri[:,:,0]
    intensity_ch = ri[:,:,1]
   
    # step 3 : set range values <0 to zero
    # The minimum range with negative values does not make sense geometrically, but instead is the value used to flag invalid points without a return signal. In order to not interfere with range image visualization and other operations such as object detection, we will set all elements with the entry -1 to 0.0 instead.
    ...
    intensity_ch = intensity_ch * 255. / (np.percentile(intensity_ch,99) - np.percentile(intensity_ch,1))
    img_intensity = intensity_ch.astype(np.uint8)
