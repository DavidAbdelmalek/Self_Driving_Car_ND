...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    ## The total number of positives present in the scene
    all_positives = 0
    false_positives = 0
    true_positives = 0
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for detected_obj in detections:
                ...
                _id, _x, _y, _z, _h, _w, _l, _yaw = detected_obj
                ...
                dist_x = float(x) - float(_x)
                dist_y = float(y) - float(_y)
                dist_z = float(z) - float(_z)  
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                #da_corners = compute_box_corners(da_x,da_y,da_w,da_l,0)
                intersection = detected_obj_poly.intersection(label_obj_poly)
                iou = intersection.area / label_obj_poly.area# check if detection area contains label object
...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            ## step : compute the total number of positives present in the scene
            all_positives+=1
            ...
            for detected_obj in detections:
                ...
                ## step 3 : extract the four corners of the current detection
                _id, _x, _y, _z, _h, _w, _l, _yaw = detected_obj
                detected_obj_corners = tools.compute_box_corners(_x,_y,_w,_l,_yaw)
                detected_obj_poly = Polygon(detected_obj_corners)
                ...
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                intersection = detected_obj_poly.intersection(label_obj_poly)
                union = label_obj_poly.union(detected_obj_poly)
                iou = intersection.area / union.area # check if detection area contains label object
